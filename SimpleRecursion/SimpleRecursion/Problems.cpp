...
void print_stack(stack<string> words) {
	if (words.empty()) 
	{ // I have an empty stack.  Nothing to do.  Base case!
		return ;
	} 
	else 
	{
...
void print_list(Node<string> *words) {
	if (words->next == NULL) 
	{ // I have reached the end of my list.  Base case!
		string word = words->data;
		cout << word + " ";
	} 
	else 
	{
		string word = words->data; // This is the word stored in the current node
		print_list(words->next);
		cout << word + " ";
	}
}
...
string reverse_string(string sentence) {
	...
	{
		char tempChar = sentence.back();
		string reversed(1,tempChar);
		sentence.pop_back();
		return reversed + reverse_string(sentence);
	}
}